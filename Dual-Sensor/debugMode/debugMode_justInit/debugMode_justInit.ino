...
// to match clock speed of teensy 3.2
//SPISettings adnsSettings(48000000, MSBFIRST, SPI_MODE3);
...
extern const prog_uchar firmware_data[];
...
void adns2_upload_firmware() {
  ...
  // send all bytes of the firmware
  // Confirmed 7/22 that firmware is being read correctly on Teensy 4
  unsigned char c;
  for (int i = 0; i < firmware_length; i++) {
    c = (unsigned char)pgm_read_byte(firmware_data + i);
    SPI.transfer(c);
